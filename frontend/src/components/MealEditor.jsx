...
const MealEditor = ({ personLabels, date, mealType, onClose, onSave, initialValues }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      // Check if click is outside both inputs and dropdown
      const isOutsidePerson1 = person1InputRef.current &&
        !person1InputRef.current.contains(event.target) &&
        activeField === 'person1';

      const isOutsidePerson2 = person2InputRef.current &&
        !person2InputRef.current.contains(event.target) &&
        activeField === 'person2';

      const isOutsideDropdown = dropdownRef.current &&
        !dropdownRef.current.contains(event.target);

      if ((isOutsidePerson1 || isOutsidePerson2) && isOutsideDropdown) {
        setActiveField(null);
        setSearchResults([]);
      }
    };
        ...
        <div>
          ...
          <div className="relative">
            ...
            {activeField === 'person1' && searchResults.length > 0 && (
              <div ref={dropdownRef} className="absolute z-10 mt-1 w-full bg-white shadow-lg rounded-md max-h-60 overflow-auto border border-downy-200">
